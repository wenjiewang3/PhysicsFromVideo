...
from loss import ode_residual_loss, ode_residual_loss_centered, ode_residual_loss_with_intercept
...
if __name__ == "__main__":
    ...
    # --- 2. Path Configuration aligned with Multi ---
    args.data_path = "syn_gen/intensity_dataset"
    result_folder = "result/syn/single_intensity" # Corresponds to multi_intensity

    # Use the same setting_case "critical" as Multi
    for setting_case in ["critical"]:

        # Summary placed in setting folder, not seed folder (aligned with Multi)
        ...
        # Summary Header adds hyperparameter columns (aligned with Multi)
        if not os.path.exists(summary_path):
            with open(summary_path, "w") as f:
                f.write("setting_case,seed,fps,second,z0,dz,encoder,tau,lr-enc,lr-ode,steps,gamma0,gamma1\n")

        # --- 3. Experiment Variable Settings (aligned with Multi) ---
        second = 2.0
        z0 = 1.0
        dz_list = [0]
