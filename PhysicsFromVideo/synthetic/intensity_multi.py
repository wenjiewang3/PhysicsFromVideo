...
from encoder import FrameEncoderCNN, FrameEncoderMLP, FrameEncoderIntensity
import torch.nn as nn
import torch.optim as optim
from loss import ode_residual_loss_with_intercept
...
def main(args):
    ...
    elif args.encoder == "intensity":
        enc = FrameEncoderIntensity().to(device)
    else:
    ...
    for step in range(1, args.steps + 1):
        ...
        loss, aux = ode_residual_loss_with_intercept(z, dt, gamma0, gamma1,
        tau=args.tau, var_weight=args.var_weight)
        ...
        if step % 2000 == 0 or step == 1 or step == args.steps:
            ...
            print(
                ...
                f"(ode={aux['loss_ode']:.4e}, var={aux['loss_var']:.4e}, c≈{aux['c_mean']:.3e}) "
...
if __name__ == "__main__":
    ...
    args.data_path = "syn_gen/intensity_dataset"
    result_folder = "result/syn/multi_intensity"
    for setting_case in ["critical"]:
        ...
        if not os.path.exists(summary_path):
            with open(summary_path, "w") as f:
                f.write("setting_case,seed,fps,second,z0, dz0, encoder, tau, lr-enc, lr-ode, steps, gamma0,gamma1\n")
        ...
        for args.fps in [10]:
            ...
            for args.seed in range(0, 5, 1):
                test_name = f"{second:.1f}_{args.fps}fps_z{z0}_dz{dz_list}"

                args.paths = (
                            f"{args.data_path}/{setting_case}/{second:.1f}_{args.fps}fps_z{z0}_dz{dz_list[0]}.mp4, "
                            f"{args.data_path}/{setting_case}/{second:.1f}_{args.fps}fps_z{z0}_dz{dz_list[1]}.mp4, "
                            f"{args.data_path}/{setting_case}/{second:.1f}_{args.fps}fps_z{z0}_dz{dz_list[2]}.mp4, "
                            )
                ...
                with open(summary_path, "a") as f:
                    f.write(
                        ...
                        f"{z0}, {dz_list}, {args.encoder}, {args.tau}, {args.lr_enc}, {args.lr_ode}, {args.steps}, {gamma0},{gamma1}\n"
                    )
